...
class testClassViewer(QMainWindow):
    ...
    def load_table_data(self):
        ...
        try:
            ...
            query = f"SELECT testClass.NumberOfClass AS 'Номер группы', testClass.IdChoreographer AS 'Номер хореографа' ,  Choreographer.Fio AS 'ФИО хореографа', testClass.DateOfClass AS 'Дата занятия', testClass.TimeOfClass AS 'Время занятия' FROM testClass JOIN Choreographer on testClass.IdChoreographer = Choreographer.IdChoreographer;"
...
class SubscriptionWindow(QMainWindow):
    ...
    def load_table_data(self):
        ...
        try:
            ...
            self.table_widget.setRowCount(len(rows))
            self.table_widget.setColumnCount(len(columns))
            custom_columns = ["Номер абонемента", "Номер клиента", "Кол-во пройденных занятий", "Кол-во занятий всего", "Дата начала занятий", "Дата окночания занятий", "Статус"]
            self.table_widget.setHorizontalHeaderLabels(custom_columns) #UPDATE
            for row_idx, row in enumerate(rows):
                for col_idx, value in enumerate(row):
                    self.table_widget.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
        except mysql.connector.Error as err:
...
class ClientsWindow(QMainWindow):
    ...
    def load_table_data(self):
        ...
        try:
            ...
            rows = cursor.fetchall()
            columns = [desc[0] for desc in cursor.description]
            custom_columns = ["Номер пользователя", "Фио", "Дата рождения", "Пол", "Номер телефона"] #UPDATE
            self.table_widget.setRowCount(len(rows))
            self.table_widget.setColumnCount(len(columns))
            self.table_widget.setHorizontalHeaderLabels(custom_columns)
...
class RecordsWindow(QMainWindow):
    ...
    def load_table_data(self):
        ...
        try:
            cursor = connection.cursor()
            query = 'SELECT FIO as "ФИО пользователя", NumberOfClass AS "Номер группы" FROM cl_rec'  # Используем представление
            cursor.execute(query)
            rows = cursor.fetchall()
            columns = [desc[0] for desc in cursor.description]
            self.table_widget.setRowCount(len(rows))
...
class ScheduleWindow(QMainWindow):
    ...
    def load_table_data(self):
        ...
        try:
            cursor = connection.cursor()
            query = f'SELECT Schedule.Id_Schedule AS "Номер расписания", Schedule.NumberOfGroup AS "Номер группы", Schedule.DateOfClass AS "Дата занятия", Schedule.TimeOfClass AS "Время занятия" FROM Schedule;'
            cursor.execute(query)
            rows = cursor.fetchall()
            columns = [desc[0] for desc in cursor.description]
            self.table_widget.setRowCount(len(rows))
    ...
    def add_schedule(self):
        ...
        try:
            cursor = connection.cursor()
            query = f"INSERT INTO Schedule (NumberOfGroup, DateOfClass, TimeOfClass) VALUES (%s, %s, %s)"
    ...
    def delete_schedule(self):
        ...
        try:
            cursor = connection.cursor()
            query = f"DELETE FROM Schedule WHERE Id_Schedule = %s"
...
class ChoreographersWindow(QMainWindow):
    ...
    def load_table_data(self):
        ...
        try:
            cursor = connection.cursor()
            query = 'SELECT Choreographer.IdChoreographer AS "Номер хореографа", Choreographer.Fio AS "ФИО", Choreographer.birthday AS "Дата рождения", Choreographer.Gender AS "Пол", Choreographer.Phone AS "Номер телефона", Choreographer.Experience AS "Стаж", Choreographer.DanceStyle AS "Танцевальный стиль", Choreographer.password AS "Пароль"FROM Choreographer'
            cursor.execute(query)
            rows = cursor.fetchall()
            columns = [desc[0] for desc in cursor.description]
